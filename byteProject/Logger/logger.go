...
func (f *CustomFormatter) Format(entry *log.Entry) ([]byte, error) {
	...
	if entry.HasCaller() {
		...
		// Make the file path relative to the project root if desired
		// (this example assumes the project root is the directory containing main.go)
		mainFilePath, err := filepath.Abs("main.go")
		if err == nil {
			mainDir := filepath.Dir(mainFilePath)
			relFile, err := filepath.Rel(mainDir, file)
			if err == nil {
				file = relFile
			}
		}
		caller = fmt.Sprintf("%s:%d %s()", file, line, runtime.FuncForPC(entry.Caller.PC).Name()[strings.LastIndex(runtime.FuncForPC(entry.Caller.PC).Name(), ".")+1:])
	} else {
